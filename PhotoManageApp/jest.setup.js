...
jest.mock('react-native-smb', () => {
  return {
    SMB2Client: jest.fn().mockImplementation((ip, username, password, shareName) => ({
      connect: jest.fn((cb) => cb(null)),
      disconnect: jest.fn((cb) => cb(null)),
      list: jest.fn((path, cb) => cb(null, [])),
      upload: jest.fn((localPath, remotePath, cb) => cb(null, 'id')),
      download: jest.fn((remotePath, localPath, cb) => cb(null, 'id')),
    })),
  };
});
